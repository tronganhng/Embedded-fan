...
void SystemInit72MHz(void) {
    ...
    // ACR = Access Control Register         : Đăng ký điều khiển truy cập Flash memory.
    // CR = Control Register                  : Đăng ký điều khiển các chức năng của RCC (Reset and Clock Control).
    // CFGR = Clock Configuration Register    : Đăng ký cấu hình đồng hồ hệ thống.
...
// ==== Delay ====
void timer_init(void) {
   RCC->APB1ENR |= RCC_APB1ENR_TIM2EN; // bật xung cho thanh ghi TIM2
   TIM2->PSC = 72 - 1;  // 72 MHz / 72 = 1 MHz (1 tick = 1 us)
   TIM2->ARR = 0xFFFF;  // 16 bit ARR (Auto-reload register): Giá trị đếm tối đa.
   TIM2->CR1 |= TIM_CR1_CEN; // bật bộ đếm
    // sau khi gọi hàm, timer luôn chạy đến ARR thì reset về 0.
}
